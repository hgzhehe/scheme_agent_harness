...
(define (load-config cwd)
  (let* ((path (path-join (sah-home) "config.scm"))
          ...
          (if (file-exists? path)
              (guard (e (#t '()))
                (let ((d (call-with-input-file path read)))
                  (if (list? d) d '())))
              '()))
         (base (list (cons 'provider 'deepseek)
                     (cons 'base-url "https://api.deepseek.com")
                     (cons 'api-key "")
                     (cons 'model "deepseek-chat")
                     (cons 'max-steps 20)
                     (cons 'system (load-system-prompt cwd))))
         ...
         (with-env (if (and (string? env-key) (not (string=? env-key "")))
                       (alist-merge with-file (list (cons 'api-key env-key)))
                       with-file)))
    with-env))
  ...
  (let loop ((args args) (opts '()) (prompt '()))
    (cond
      ...
      ((string=? (car args) "--repl")
       (loop (cdr args) (cons (cons 'mode 'repl) opts) prompt))
      ((or (string=? (car args) "-C") (string=? (car args) "--continue"))
       (loop (cdr args) (cons (cons 'continue #t) opts) prompt))
      ((or (string=? (car args) "-H") (string=? (car args) "--usage")
           (string=? (car args) "-h") (string=? (car args) "--help"))
       (loop (cdr args) (cons (cons 'help #t) opts) prompt))
      ((string=? (car args) "--key")
       (loop (cddr args) (cons (cons 'api-key (cadr args)) opts) prompt))
      ((string=? (car args) "--model")
       (loop (cddr args) (cons (cons 'model (cadr args)) opts) prompt))
      ((string=? (car args) "--base-url")
       (loop (cddr args) (cons (cons 'base-url (cadr args)) opts) prompt))
      ((string=? (car args) "--max-steps")
       (loop (cddr args) (cons (cons 'max-steps (string->number (cadr args))) opts) prompt))
