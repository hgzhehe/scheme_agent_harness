;;; manager.ss -- session persistence in SexprL (one Scheme datum per line).
;;;
;;; Files live under ~/.sah/sessions/<cwd-slug>/<unix-ms>_<id>.ss and every line
;;; is a Scheme datum readable with `read` (see core/data.ss for the shapes).
;;;
;;; Appends are O(1): entries are held newest-first in memory (append is a cons)
;;; and the output port is held open (append is one write, not a whole-file
;;; rewrite). A session loaded from disk opens its port lazily on the first
;;; append (one rewrite, then O(1)). The on-disk form is a tree via id/parent,
;;; so branches can land later without a format change. Entries written by older
