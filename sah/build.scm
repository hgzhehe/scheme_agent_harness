...
;;; Most distributions therefore need only exe + boot; Windows builds whose
;;; runtime links local DLLs carry those sidecars too. A true single-file exe
...
(define (combined-source)
  (let ((code (source-text)))
    (string-append
     ...
     "(import (chezscheme))\n"
     code
     "\n;;; ---- populate the plugin environment ----\n"
...
;; Some Windows Chez distributions keep the Scheme kernel and iconv beside the
;; tiny runtime executable as DLLs. Copy every local DLL rather than hardcoding
;; versioned names such as csv1050.dll.
(define (copy-runtime-sidecars! runtime-exe dist-dir)
  (when windows?
    (let* ((runtime-dir (dirname runtime-exe))
           (sidecars
            (filter (lambda (name)
                      (string-suffix? ".dll" (string-downcase name)))
                    (dir-entries runtime-dir))))
      (for-each
       (lambda (name)
...
(define (shell-path path)
  ;; `system` uses cmd.exe on Windows, where a quoted executable path beginning
  ;; with `./` is not resolved as a command. The runtime APIs accept either
  ;; separator, but the smoke-check shell needs the native spelling.
  (if windows?
      (list->string
       (map (lambda (c) (if (char=? c #\/) #\\ c)) (string->list path)))
      path))
