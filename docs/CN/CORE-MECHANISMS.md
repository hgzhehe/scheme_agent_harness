...
重放本身也会失败：journal 里的表单可能依赖录制时存在、现在已经不存在的环境
（插件改过版、宿主绑定消失、import 被移除）。这时两种调用者的含义不同：

- **加载或重建会话**：跳过该表单并在 stderr 逐条报出（`scope form not replayed: …`），
  其余表单照常生效，会话仍然打开。一个过期表单不应让一份 journal 永久无法打开。
- **插件集变更探测**：比较变更前后的"无法重放表单集合"，只有**新增**的失败才算
  "当前会话仍依赖这套插件"，据此拒绝并回滚（见 `extend/loader.ss` 的
  `runtime-change-plugin!`）。既有失败不算证据，否则任何过期表单都会让 dispose
  永远失败。
