...
>
> **Divergences from this plan.** The implementation went a different way in
> several
...
> - `modes/` now includes a fullscreen TUI, line REPL, print/JSON output and a
>   JSONL RPC frontend over one shared session host
...
> - plugin programs use explicit op/frame transactions and can contribute
>   renderers and TUI widgets
> - `config.scm` is read as an alist datum rather than evaluated as a program
